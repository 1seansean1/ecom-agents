---
name: next
description: "Execute the next critical-path task in the Holly Grace development pipeline. Runs the full Task Execution Checklist: pre-flight audit, state sync, task identification, implementation, test authoring, doc sync, post-flight audit, and commit. Use when: user says 'next task', 'continue building', 'next on critical path', or invokes /next."
---
